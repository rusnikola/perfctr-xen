				     ...
				     struct vperfctr *perfctr)
{
	...
	spin_lock_irqsave(&task_siglock(tsk), flags);
	old_blocked = tsk->blocked;
	sigaddset(&tsk->blocked, si.si_signo);
	spin_unlock_irqrestore(&task_siglock(tsk), flags);
...
static char *vperfctrfs_dname_dentry(struct dentry *dentry, char *buffer, int buflen)
{
	return dynamic_dname(dentry, buffer, buflen, "vperfctr:%s",
		dentry->d_name.name);
}
...
struct dentry_operations vperfctrfs_dentry_operations = {
	.d_dname	= vperfctrfs_dname_dentry,
};

static struct dentry *vperfctrfs_mount(struct file_system_type *fs_type,
	int flags, const char *dev_name, void *data)
{
	return mount_pseudo(fs_type, "vperfctr:", NULL,
		&vperfctrfs_dentry_operations, VPERFCTRFS_MAGIC);
}

static struct file_system_type vperfctrfs_type = {
	.name		= "vperfctrfs",
	.mount		= vperfctrfs_mount,
...
static struct inode *vperfctr_get_inode(void)
{
	...
	inode = new_inode_pseudo(vperfctr_mnt->mnt_sb);
	if (!inode)
		return NULL;
...
static struct dentry *vperfctr_d_alloc_root(struct inode *inode)
{
	...
	this.hash = 0;
	dentry = d_alloc_pseudo(vperfctr_mnt->mnt_sb, &this);
	return dentry;
}
...
static struct file *vperfctr_get_filp(void)
{
	...
	if (!inode)
		goto out;
	path.dentry = vperfctr_d_alloc_root(inode);
	if (!path.dentry)
		goto out_inode;
	/*
	 ...
	 */
	path.mnt = mntget(vperfctr_mnt);
	d_instantiate(path.dentry, inode);
	filp = alloc_file(&path, FMODE_READ, &vperfctr_file_ops);
	if (!filp)
		goto out_path;
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,2)
	filp->f_mapping = path.dentry->d_inode->i_mapping;
#endif
